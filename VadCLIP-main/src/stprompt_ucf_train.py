...
from stprompt_ucf_test import test
...
def train(model, train_loader, test_loader, args, label_map: dict, device):
    ...
    for category in label_map.values():
        if category != "Normal":
            ...
            if category == "Abuse":
                ...
                abnormal_texts.extend(["police detaining someone", "handcuffing", "law enforcement action"])
            elif category == "Arson":
                abnormal_texts.extend(["someone ignite fire", "burning building", "deliberate fire"])
            elif category == "Assault":
                abnormal_texts.extend(["physical attack", "violent confrontation", "aggressive behavior"])
            elif category == "Burglary":
                abnormal_texts.extend(["breaking into building", "theft", "unauthorized entry"])
            elif category == "Explosion":
                abnormal_texts.extend(["blast", "detonation", "explosive event"])
            elif category == "Fighting":
                abnormal_texts.extend(["people knockout someone", "physical altercation", "violent struggle"])
            elif category == "RoadAccident":
                abnormal_texts.extend(["car crash", "vehicle collision", "traffic incident"])
            elif category == "Robbery":
                abnormal_texts.extend(["theft with force", "armed robbery", "stealing valuables"])
            elif category == "Shooting":
                abnormal_texts.extend(["people shooting someone", "gunfire", "armed violence"])
            elif category == "Shoplifting":
                abnormal_texts.extend(["stealing from store", "retail theft", "concealing merchandise"])
            elif category == "Stealing":
                abnormal_texts.extend(["taking property", "theft action", "unauthorized removal"])
            elif category == "Vandalism":
                abnormal_texts.extend(["property damage", "destructive behavior", "defacing property"])
    ...
    for e in range(args.max_epoch):
        ...
        scheduler.step()
        AUC, AP = test(model, test_loader, args.visual_length, prompt_text, gt, gtsegments, gtlabels, device, normal_texts, abnormal_texts)
...
if __name__ == '__main__':
    ...
    args.model_path = 'model/model_stprompt_ucf.pth'
    args.checkpoint_path = 'model/stprompt_ucf_checkpoint.pth'
    args.train_list = 'list/ucf_CLIP_rgb.csv'
    args.test_list = 'list/ucf_CLIP_rgbtest.csv'
    args.gt_path = 'list/gt_ucf.npy'
    args.gt_segment_path = 'list/gt_segment_ucf.npy'
    args.gt_label_path = 'list/gt_label_ucf.npy'
    args.classes_num = 14
    setup_seed(args.seed)

    label_map = dict({'Normal': 'Normal', 'Abuse': 'Abuse', 'Arrest': 'Arrest', 'Arson': 'Arson', 'Assault': 'Assault',
                      'Burglary': 'Burglary', 'Explosion': 'Explosion', 'Fighting': 'Fighting', 'RoadAccident': 'RoadAccident',
                      'Robbery': 'Robbery', 'Shooting': 'Shooting', 'Shoplifting': 'Shoplifting', 'Stealing': 'Stealing',
                      'Vandalism': 'Vandalism'})

    train_dataset = UCFDataset(args.visual_length, args.train_list, False, label_map)
